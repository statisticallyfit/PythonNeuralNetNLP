...
Each tuple is ({[} \mintinline[]{python}{word}\(_{i-2}\),
\mintinline[]{python}{word}\(_{i-1}\) {]},
\mintinline[]{python}{targetWord})
...
\mintinline[]{python}{set} to remove duplicate words.
...
\item
  ...
  \mintinline[]{python}{nn.Embedding}, to convert tensors into word
  embeddings.
\item
...
\tightlist
\item
  Convert the tensor \mintinline[]{python}{inputs} to word embeddings
  via the skip-gram's \mintinline[]{python}{nn.Embedding} layer
\item
  Pass the embeddings to the hidden layer and transform the result using
  the \mintinline[]{python}{relu} function
\item
  Transform the hidden layer results using the output layer.
\item
  Finally, create a probability distribution over words using the
  \mintinline[]{python}{softmax} function. (Here we actually use the
  \mintinline[]{python}{log_softmax} so the results are log
...
\mintinline[]{python}{forward} pass of the skip-gram model and get the
...
\item
  ...
  \mintinline[]{python}{wordToIndex} dictionary, and make their type a
  \mintinline[]{python}{Tensor}.
\item
  Set the model gradients to zero so they do not accumulate artificially
  (feature of pytorch)
\item
  Do the \mintinline[]{python}{forward} pass of the Skip-Gram model,
...
\item
  ...
  index using the \mintinline[]{python}{wordToIndex} dictionary and wrap
  it in a \mintinline[]{python}{Tensor} type.
\item
  Compute the loss between the log probabilities and target contexts.
\item
  Do the \mintinline[]{python}{backward} pass over the neural network to
  update the gradients by calling
  \mintinline[]{python}{loss.backward()}.
\item
