...
# note: Loss function optimization: want to minimize the difference between the
# note: network predictions and actual labels' values (using cross entropy to define the loss)

# note: tensorflow's tf.nn.softmax_cross_entropy_with_logits applies softmax on model's
# note unnormalized prediction and sums across all classes. The tf.reduce_mean takes
# note the average over these sums.
# # help: That means we can further optimize the function.
